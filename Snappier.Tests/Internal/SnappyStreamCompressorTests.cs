...
namespace Snappier.Tests.Internal
{
    public class SnappyStreamCompressorTests
    {
        ...
        public void Write(string resourceName)
        {
            ...
            var input = memStream.GetBuffer().AsSpan(0, (int) memStream.Length);
            ...
            var decompressed = new byte[memStream.Length + 1]; // Add 1 to make sure decompress ends correctly
            var bytesDecompressed = decompressor.Decompress(decompressed);
